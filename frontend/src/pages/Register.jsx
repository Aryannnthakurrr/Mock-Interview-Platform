...
import { Mail, Lock, User, Eye, EyeOff, Flame, ArrowRight, GraduationCap, ShieldCheck } from 'lucide-react';
            ...
            <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div className="glass-card p-8 md:p-10">
                    <div className="text-center mb-8">
                        ...
                        <h1 className="text-2xl font-bold mb-2 text-white-50">Create Account</h1>
                        <p className="text-sm text-blue-50/60">Start your career acceleration journey</p>
                    </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            {[
                                ...
                                <button key={r.value} type="button" onClick={() => setRole(r.value)}
                                    className={`p-3 rounded-xl text-sm font-medium flex items-center justify-center gap-2 transition-all duration-300 ${role === r.value
                                            ? 'bg-white-50 text-black-100 shadow-lg'
                                            : 'bg-black-200 text-blue-50 hover:bg-blue-100'
                                        }`}
                                ><r.icon className="w-4 h-4" />{r.label}</button>
                            ))}
                        </div>

                        <div>
                            <label className="block text-sm font-medium mb-2 text-blue-50">Full Name</label>
                            <div className="relative">
                                <User className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-blue-50/40" />
                                <input type="text" value={name} onChange={(e) => setName(e.target.value)} placeholder="Keshav Goel" className={inputClass} />
                            </div>
                        </div>
                        <div>
                    ...
                    <div className="mt-6 text-center text-sm text-blue-50/50">
                        ...
                        <Link to="/login" className="text-white-50 hover:underline font-medium">Sign In</Link>
                    </div>
                </div>
            </motion.div>
        </div>
